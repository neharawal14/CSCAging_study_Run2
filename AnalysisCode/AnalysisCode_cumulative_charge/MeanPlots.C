...
void MeanPlot :: initialise(std::vector<	std::pair<float, float>> mean_vector,
   std::vector<TString> chamber, TString year_value , TString var_name){

    var= var_name;
    result_path="/afs/cern.ch/work/n/nrawal/CSCAgeing_code_study/results_AnalysisCode/New_selections_2017/MeanPlots/";
    year = year_value;
...
// end of initialisation function 
  ...
  void MeanPlot :: plot_mean(){
    ...
    if(var=="_pressure")    graph_mean_values->GetYaxis()->SetRangeUser(-0.010,0.002); 
    else if(var=="_instlumi")    graph_mean_values->GetYaxis()->SetRangeUser(-0.00001,0.00001); 
    else graph_mean_values->GetYaxis()->SetRangeUser(-0.0002,0.0002);
    ...
    std::cout<<" declaring gr x arrayr "<<std::endl;
        ...
        m->SetMarkerColor(marker_colour[j]);
        m->Draw();
...
//      text2->Draw("same");
	 ...
	 canv1->SaveAs(result_path+"mean_slope_values_"+var+"_"+year+".pdf"); 
  }
