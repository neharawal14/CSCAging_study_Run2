...
parser.add_argument("year_name", help="dataset year")
...
try:
  ...
  input_text_file = "../final_HV_info_files/HV_info_golden_ME11_asc"+year+".txt"
  ...
  open_file_string = "../DPID_project/dpid_name_ME11.txt"
  print(" started running after opening dpid ")
  ...
  for value_dpid in f_dpid_values:
    ...
    for i in range(len(run_nb_list)):
      ...
      query_start = "SELECT ACTUAL_VMON, CHANGE_DATE FROM CMS_CSC_PVSS_COND.FWCAENCHANNEL WHERE CHANGE_DATE <  '"+str(date_start_list) + "'  AND DPID="+dpid_value+" AND ACTUAL_VMON IS NOT NULL ORDER BY CHANGE_DATE DESC FETCH FIRST 1 ROWS ONLY" ; 
      query_middle = "SELECT ACTUAL_VMON, CHANGE_DATE FROM CMS_CSC_PVSS_COND.FWCAENCHANNEL WHERE DPID="+dpid_value+" AND CHANGE_DATE between '"+date_start_list+"' AND '"+str(date_end_list)+"' AND ACTUAL_VMON IS NOT NULL ORDER BY CHANGE_DATE ASC"; 
