...
/*  
		 TH3D * hchargevstime = new TH3D("hchargevstime","charge (ADC counts) vs time ",3000,0,3000,60, 1656633600,1672444800,770,1,771);
...
// after pressure correction
 /*  TH3D * hchargevsintegratelumi_first_correction = new TH3D("hchargevsintegratelumi_first_correction","charge (ADC counts) vs integ lumi (initial)",3000,0,3000, 40, 0,40  ,770,1,771);
   ...
   //    if(i%100 !=0 &&istest) continue;
     ...
     double charge  = _rhsumQ_RAW* ApplyCorrection( _pressure ,"pressure",   (params_pressure[0]).first , (params_pressure[0]).second )  ;
	   ...
	   if(debug_print) std::cout<<" This event entering exit "<<i<<std::endl;
     hchargevsintegratelumi_first_correction->Fill(charge, _integratelumi, rhidreduced);
   }
...
//            h_prov_new->SetBins(150,-0.5,2999.5);
       	    ...
       	    if(h_prov->Integral()<50) continue;
		        normal = h_prov->Integral();
						// first normalize a histogram with Integral
					//	h_prov->ResetStats();
    				int last_bin = 0;
						double integral =0;
...
//				 h_trim_new->SetBins(150,-0.5,2999.5);
					h_trim_new->GetXaxis()->SetRange(0,0);	
					h_trim->GetXaxis()->SetRange(0,0);	
//				 std::cout<<" entries in the original histogram : rhid"<<rhidshort<<" : bin "<<j<<" entries "<<h_trim->GetEntries()<<" integral "<<h_trim->Integral()<<" mean "<<h_trim->GetMean()<<std::endl;
//				 std::cout<<" entries in the new  cleaned histogram : rhid"<<rhidshort<<" : bin "<<j<<" entries "<<h_trim_new->GetEntries()<<std::endl;
...
/*    	 if(h_trim->Integral()>0 && (thevar.Index("integratelumi")>=0) && rhidshort =="allgoodchannels"){
   	 ...
   	 if(h_trim_new->Integral()>0 && (thevar.Index("integratelumi")>=0 || thevar.Index("pressure") >=0 || thevar.Index("instlumi")>=0) &&savehistos && (rhidshort=="chamber36_layer6_Endcap2" || rhidshort =="chamber9_layer1_Endcap1" || (rhidshort=="allgoodchannels" && j==2) )){
			 	 TCanvas *canvas_charge = new TCanvas();
 			 	 canvas_charge->cd();
	    	 h_trim_new->Draw("E");
 			 	 canvas_charge->SaveAs(output_plots_folder+"plotfolder_"+chamber_string_name+"/charge_distribution"+title+"_"+rhidshort+"vs"+thevar+"_bin"+j+"_after_trim.pdf"); 
...
*/
			     // we are fillling entries only if atleast a distribution has more than 50 entries
...
//		htrimmeanvsX->Fit(fa1, "R"); 
    ...
    c3->SetName("c_"+filename+"trimmean"+title+"_"+rhidshort+"vs"+thevar);
       ...
       htrimmeanvsX->Draw();
			...
			//if( (thevar.Index("instlumi") >=0&& thevar.Index("instlumi_2") <0 && theslope>0.01 ) || (thevar.Index("pressure") >=0 && thevar.Index("pressure_2") >=0  && theslope>0)   ){
...
/*
void ProduceHistosPerChannel::gas_gain_time(){
	float time[40] = {07/29/22 ,08/02/22 ,08/05/22 ,08/09/22 ,08/10/22 ,08/13/22 ,08/17/22 ,08/20/22 ,08/21/22 ,08/23/22 ,10/02/22 ,10/03/22 ,10/06/22 ,10/08/22 ,10/10/22 ,10/13/22 ,10/15/22 ,10/16/22 ,10/20/22 ,10/22/22 ,10/23/22 ,10/25/22 ,10/26/22 ,10/27/22 ,10/28/22 ,10/29/22 ,10/31/22 ,11/01/22 ,11/02/22 ,11/03/22 ,11/05/22 ,11/12/22 ,11/14/22 ,11/15/22 ,11/16/22 ,11/21/22 ,11/24/22 ,11/26/22 ,11/27/22}; 
  ...
  dt.Set(time[i]);
	t= dt.Convert();
	gr->SetPoint(i, gas_gain_vector[i], t);
	gr->SetPointError(i, gas_gain_error_vector[i], 0);
	}
	TCanvas * c = new TCanvas("c"," canvas");
	c->cd();
	gr->Draw();
	c->SaveAs("canvas_gas_gain.pdf");
	c->Close();
