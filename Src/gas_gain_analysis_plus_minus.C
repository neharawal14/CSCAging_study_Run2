#define gas_gain_analysis_plus_minus_cxx
#include "gas_gain_analysis_plus_minus.h"
...
/*void gas_gain_analysis_plus_minus::z_distribution(){

}*/
void gas_gain_analysis_plus_minus::Loop(TString input_file_path, TString input_file_name, TString chamber_string, TString output_file_path, TString output_folder_name)
{
  ...
  iszmumu = false;
   ...
   TFile * outf  = new TFile(output_path+"outf"+detregionstr+"_"+chamber_string_name+"_plus_output.root","recreate");  
 ...
 */   
 	 ...
 	 TFile *file_output = new TFile(output_path+"output_after_pressure_depenedence_removal_"+chamber_string_name +"_plus.root","RECREATE");
...
/*
std::vector<float> gas_gain_analysis_plus_minus :: trimmed_mean(TH2D *myh){
...
*/
vector < std::pair<double, double > >  gas_gain_analysis_plus_minus::GetSlope( TH3D * myh , TString thevar , TString filename, TString title, TFile * outf){
...
double gas_gain_analysis_plus_minus::ApplyCorrection( double X ,TString correctiontype, double p0, double p1 ){
  double refvalue = 0;
